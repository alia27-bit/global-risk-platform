...
class SentimentService
{
    ...
    {
        $positive = $this->lexicon(
            PositiveWord::query()->pluck('word')->all(),
            self::POSITIVE_FALLBACK
        );
        $negative = $this->lexicon(
            NegativeWord::query()->pluck('word')->all(),
            self::NEGATIVE_FALLBACK
        );
        ...
        foreach ($words as $word) {
            ...
            if (array_intersect($forms, $positive)) {
                $positiveScore++;
                $matches[] = ['word' => $word, 'type' => 'positive'];
            }
            if (array_intersect($forms, $negative)) {
                $negativeScore++;
                $matches[] = ['word' => $word, 'type' => 'negative'];
            }
        }
        ...
        return [
            ...
            'neutral_percentage' => $matched ? 0.0 : 100.0,
            'matches' => $matches,
        ];
    }
    ...
    private function forms(string $word): array
    {
        ...
        foreach (['ing', 'ed', 'es', 's'] as $suffix) {
            if (mb_strlen($word) > mb_strlen($suffix) + 2 && str_ends_with($word, $suffix)) {
                $forms[] = mb_substr($word, 0, -mb_strlen($suffix));
            }
        }
